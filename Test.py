...
if __name__ == '__main__':
    ...
    if args.dataset == 'RSOC_building':
        ...
        test_dataset = listDataset(test_list, train=False, num_workers=args.num_workers)
    elif args.dataset == 'VisDronePeople':
        ground_path = args.test_dir.replace('images', 'Ground_Truth')
        test_dataset = cdpeopleDataset(args.test_dir, ground_path, train=False)
    elif args.dataset == 'VisDroneVehicle':
        ground_path = args.test_dir.replace('images', 'Ground_Truth')
        test_dataset = cdvehicleDataset(args.test_dir, ground_path, train=False)
    else:
    ...
    assert os.path.exists('./eFateNet.pth'), "file: '{}' dose not exist.".format('./eFateNet.pth')
