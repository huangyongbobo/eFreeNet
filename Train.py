...
def train(args, model, loss_function, loss_function2, optimizer, epoch, device):
    if args.dataset == 'RSOC_building':
        ...
        train_dataset = listDataset(train_list, transform=Train_transform, train=True, num_workers=args.num_workers)
    elif args.dataset == 'VisDronePeople':
        Train_transform = transforms.Compose([transforms.Resize((512, 768)),
                                              transforms.ToTensor(),
                                              transforms.Normalize(mean=[0.485, 0.456, 0.406],
                                                                   std=[0.229, 0.224, 0.225]),
                                              ])
        ground_path = args.train_dir.replace('images', 'Ground_Truth')
        train_dataset = cdpeopleDataset(args.train_dir, ground_path, Train_transform, train=True)
        ...
        ground_path = args.train_dir.replace('images', 'Ground_Truth')
        train_dataset = cdvehicleDataset(args.train_dir, ground_path, Train_transform, train=True)
    else:
...
def validate(args, model, epoch, device):
    ...
    if args.dataset == 'RSOC_building':
        ...
        val_dataset = listDataset(val_list, transform=val_transform, train=False, num_workers=args.num_workers)
    elif args.dataset == 'VisDronePeople':
        val_transform = transforms.Compose([transforms.Resize((512, 768)),
                                            transforms.ToTensor(),
                                            transforms.Normalize(mean=[0.485, 0.456, 0.406],
                                                                 std=[0.229, 0.224, 0.225]),
                                            ])
        ground_path = args.val_dir.replace('images', 'Ground_Truth')
        val_dataset = cdpeopleDataset(args.val_dir, ground_path, val_transform, train=False)
    elif args.dataset == 'VisDroneVehicle':
        val_transform = transforms.Compose([transforms.Resize((512, 768)),
                                            transforms.ToTensor(),
                                            transforms.Normalize(mean=[0.485, 0.456, 0.406],
        ...
        ground_path = args.val_dir.replace('images', 'Ground_Truth')
        val_dataset = cdvehicleDataset(args.val_dir, ground_path, val_transform, train=False)
    else:
